...
def simple_CNN(input_shape, num_labels):
	...
	model.add(BatchNormalization())
	model.add(Convolution2D(filters=num_labels, kernel_size=(3, 3), padding='same'))
	model.add(GlobalAveragePooling2D())
	model.add(Activation('softmax',name='predictions'))
	return model

def simpler_CNN(input_shape, num_labels):
...
def tiny_XCEPTION(input_shape, num_labels, l2_regularization=0.01):
...
def mini_XCEPTION(input_shape, num_labels, l2_regularization=0.01):
	regularization = l2(l2_regularization)
...
def big_XCEPTION(input_shape, num_labels):
	img_input = Input(input_shape)
...
if __name__ == "__main__":
	input_shape = (64, 64, 1)
	num_labels = 7
	#model = tiny_XCEPTION(input_shape, num_labels)
	#model.summary()
	#model = mini_XCEPTION(input_shape, num_labels)
	#model.summary()
	#model = big_XCEPTION(input_shape, num_labels)
	#model.summary()
	model = simple_CNN((48, 48, 1), num_labels)
	model.summary()
